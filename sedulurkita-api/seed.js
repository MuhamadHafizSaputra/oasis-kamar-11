...
async function seedDatabase() {
  try {
    ...
    await db.query('TRUNCATE TABLE reviews, umkm RESTART IDENTITY CASCADE');
    console.log('Cleared old tables...');

    // 2. Loop semua data di mockData
    for (const umkm of umkmData) {
      console.log(`Inserting ${umkm.name}...`);

      // 3. Masukkan ke tabel 'umkm'
      const umkmInsertQuery = `
        INSERT INTO umkm (
          id, name, category, subcategory, phone, 
          price_min, price_max, payment_methods, last_updated, 
          owner, rating, reviews, location, distance, price_from, 
          latitude, longitude, images, description, details
        ) 
        VALUES (
      ...
      const umkmValues = [
        umkm.id, umkm.name, umkm.category, umkm.subcategory, umkm.phone,
        umkm.priceRange ? umkm.priceRange[0] : null, // price_min
        umkm.priceRange ? umkm.priceRange[1] : null, // price_max
        JSON.stringify(umkm.paymentMethods),         // payment_methods (JSON)
        umkm.lastUpdated,                             // last_updated
        JSON.stringify(umkm.owner),                   // owner (JSON)
        umkm.rating, umkm.reviews, umkm.location, umkm.distance, umkm.priceFrom,
        umkm.latitude, umkm.longitude,
        JSON.stringify(umkm.images),                  // images (JSON)
        umkm.description,
        JSON.stringify(umkm.details)                  // details (JSON)
      ];

      const result = await db.query(umkmInsertQuery, umkmValues);
      const newUmkmId = result.rows[0].id;

      // 4. Masukkan ke tabel 'reviews'
      if (umkm.reviewsList) {
        for (const review of umkm.reviewsList) {
          const reviewInsertQuery = `
            INSERT INTO reviews (
              review_id, user_name, rating, text, date, umkm_id
            ) 
            VALUES ($1, $2, $3, $4, $5, $6);
          `;
          // Ganti 'review.user' menjadi 'review.user_name' jika di DB
          const reviewValues = [
            review.id, review.user, review.rating, 
            review.text, review.date, newUmkmId
          ];
          await db.query(reviewInsertQuery, reviewValues);
        }
      }
    }
